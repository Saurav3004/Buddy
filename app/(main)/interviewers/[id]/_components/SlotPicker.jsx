...
const SlotPicker = ({ interviewer, interviewerCredits, userCredits }) => {
  ...
  const slots = useMemo(() => {
    ...
    return generateSlots(selectedDate, availability.startTime, availability.endTime, interviewer.bookingAsInterviewer ?? [], SLOT_DURATION_MINUTES);
  ...
  return (
    <>
      <UpgradeModal open={upgradeOpen} onOpenChange={setUpgradeOpen} reason={`You need ${interviewerCredits} credits to book this session. Your current balance is ${userCredits}.`} />
      ...
      <div className='flex flex-col gap-4'>
        ...
        <div className='bg-[#0f0f11] border border-white/10 rounded-2xl p-7 flex flex-col gap-6'>
          <div className='flex  items-start justify-between gap-3'>
            ...
            <div className='text-right shrink-0'>
              ...
              <p className='font-serif text-2xl leading-none bg-linear-to-br from-blue-300 to-blue-500 bg-clip-text text-transparent'>{interviewerCredits}
          ...
          <div className='flex gap-2 overflow-x-auto pb-0.5 scrollbar-none -mx-1 px-1'>
            {dates.map((date) => {
              ...
              return (
                <button key={date.toDateString()} type='button' onClick={() => handleDateChange(date)} className={`shrink-0 flex flex-col items-center px-3.5 py-2.5 rounded-xl border text-xs transition-all duration-200 ${active ? "border-blue-400/40 bg-blue-400/10 text-blue-400" : "border-white/10 text-stone-500 hover:border-white/20 hover:text-stone-400 cursor-pointer"}`}>
                  <span className='font-medium'>{label.top}</span>
                  <span className={`mt=0.5 ${active ? "text-blue-500/70" : "text-stone-700"}`}>
                    {label.bottom}
                  </span>
                </button>
              )
          ...
          {slots.length === 0 ? (
            ...
            <div className='grid grid-cols-3 gap-2'>
              {slots.map((slot) => {
                ...
                return (
                  <button key={slot.startTime.toISOString()} type='button' onClick={() => handleSlotClick(slot)} disabled={slot.isBooked} className={`relative px-2 py-2.5 rounded-xl border text-xs transition-all duration-200 ${isSelected ? "border-blue-400/60 bg-blue-400/15 text-blue-300 shadow-[0_0_12px_rgba(251,191,36,0.08)]" : slot.isBooked ? "border-white/5 bg-white/2 text-stone-700 cursor-not-allowed" : "border-white/10 text-stone-400 hover:border-blue-400/30 hover:text-blue-400 hover:bg-blue-400/5 cursor-pointer"}`}>
                    {formatTime(slot.startTime)}
                    {slot.isBooked && (
                      <span className='absolute inset-x-0 bottom-0.5 text-center text-stone-700 leading-none' style={{ fontSize: "9px" }}>
                        booked
                      </span>
                    )}
                  </button>
        ...
        {selectedSlot && (<div ref={summaryRef} className='bg-[#0f0f11] border border-blue-400/20 rounded-2xl p-6 flex flex-col gap-4'>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-serif text-lg bg-linear-to-br from-blue-300 to-blue-500 bg-clip-text text-transparent leading-none">
              −{interviewerCredits}
            </span>
          </div>
          <div className="flex justify-between text-xs">
