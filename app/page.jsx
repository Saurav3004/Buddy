...
import { GoldTitle, GrayTitle, SectionHeading, SectionLabel } from "@/components/reusable";
...
export default function Home() {
  ...
  return (
    <div className="bg-black overflow-x-hidden">
      ...
      <section className="pt-28 sm:pt-32 relative min-h-screen grid grid-col-1 lg:grid-cols-5 px-4 sm:px-8 pb-20 overflow-hidden ">
        ...
        <div className="col-span-full lg:col-span-3 flex flex-col items-center justify-center text-center lg:-rotate-2">
          ...
          <div className="relative flex items-center justify-center gap-3 sm:gap-4 mt-8 sm:mt-16">
            <div className="flex">
              {AVATARS.map((av, i) => (
                <div key={i} className={`w-10 h-10 rounded-full border-2 border-[#0a0a0b] overflow-hidden ${i > 0 ? "-ml-2" : ""}`}>
                  <Image src={av.src} alt="user avatar" width={42} height={42} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
            <p className="text-sm text-stone-500 text-center sm:text-left">
              <strong className="text-stone-400 font-medium">2400+ engineers</strong>{" "}
              cracked MAANG interviews via Buddy
            </p>
          </div>
        </div>
      ...
      <section className="relative z-10 border-y border-white/10 py-14">
        <p className="text-center text-xs font-medium text-stone-600 tracking-widest uppercase mb-8">Interviews landed roles at</p>
      ...
      <section className="relative z-10 py-28 max-w-5xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <SectionHeading gray={"Everything you need,"} blue={"nothing you don't"} />
        </div>

        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 md:col-span-7">
            <BentoCard icon={<Bot size={20} className="text-blue-400" />} title={<GrayTitle>AI Question Generator</GrayTitle>} desc={"Interviewers get a live AI co-pilot generating role-specific questions on demand - system design, behavioural, DSA - all tailored to the candidate's level."}>
          ...
          <div className="col-span-12 md:col-span-5">
            <BentoCard icon={<Wallet size={20} className="text-blue-400" />} title={<GrayTitle>Credit System</GrayTitle>} desc={"Subscribe for monthly credits. Book sessions. Interviewers earn and withdraw any time."}>
              <div className="mt-5 rounded-xl bg-[#141417] border border-white/10 p-5 flex justify-between items-end">
                <div>
                  ...
                  <p className="font-serif text-4xl leading-none bg-linear-to-br from-blue-300 to-blue-500 bg-clip-text text-transparent">
                    28
                  </p>
      ...
      <section className="relative z-10 pb-28 max-w-5xl mx-auto px-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {ROLES.map((role) => (
                    <div key={role.label} className="relative bg-[#0f0f11] border border-white/10 hover:border-blue-400/20 rounded-2xl p-12 h-full transition duration-300 overflow-hidden">
                      <span className="inline-block text-xs font-semibold text-blue-400 tracking-widest uppercase border border-blue-400/20 bg-blue-400/10 rounded-full px-3 py-1.5 mb-1.5">
                        {role.label}
                      </span>

                      <h3 className="font-serif text-2xl tracking-tight mb-4">{role.title}</h3>
                      ...
                      <ul className="space-y-3">
                        {
                          role.perks.map((p) => (
                            <li key={p} className="flex gap-3 text-sm text-stone-400">
                              <span className="mt-0.5 min-w-4 h-4 rounded-full bg-blue-400/10 border border-blue-400/20 flex items-center justify-center text-xs text-blue-400">
                              ✓
                              </span>
                              {p}
                            </li>
                  ...
                  <section className="relative z-10 pb-28 max-w-5xl mx-auto px-6">
                    <div className="relative border border-blue-400/20 rounded-3xl px-3 sm:px-16 py-20 bg-linear-to-br from-blue-400/5 text-center overflow-hidden">
                    <GravityStarsBackgroundDemo />
                    ...
                    <p className="relative text-stone-400 font-light text-sm mb-11">
                      Join thousands of engineers already levelling up on Buddy.
                    </p>
