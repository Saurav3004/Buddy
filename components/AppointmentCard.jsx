...
export function AppointmentCard({ booking, mode, isPast = false }) {
  ...
  const creditsLabel =
    mode === "interviewer"
      ? `+${creditCharged} credits earned`
      : `−${creditCharged} ${creditCharged > 1 ? 'credits' : 'credit'}`;
  ...
  return (
    <>
      ...
      <article className="group relative bg-[#0f0f11] border border-white/10 transition-all duration-300 hover:-translate-y-0.5 rounded-2xl bg-linear-to-t from-transparent via-transparent to-blue-300/10 p-7 flex flex-col gap-6 self-start">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            <Avatar className="w-14 h-14 border border-white/10 rounded-2xl shrink-0">
              ...
              />
              <AvatarFallback className="rounded-2xl bg-blue-400/10 border border-blue-400/20 text-blue-400 text-lg font-medium">
                {person?.name?.[0] ?? "?"}
              </AvatarFallback>
            </Avatar>
              ...
              {mode === "interviewee" && person?.categories?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {person.categories.slice(0, 3).map((cat) => (
                    <span
                      key={cat}
                      className="text-[10px] px-2 py-0.5 rounded-md border border-blue-400/20 bg-blue-400/5 text-blue-400 leading-tight"
                    >
                      {cat.replace("_", " ")}
                    </span>
                  ))}
              ...
              (has?.({ plan: "starter" }) || has?.({ plan: "pro" })) && (
                <>
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-1.5 border-blue-400/20 text-blue-400 hover:bg-blue-400/10 hover:border-blue-400/40"
