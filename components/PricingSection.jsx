        ...
        return (
          <div
            ...
            className={`relative rounded-2xl p-10 h-full flex flex-col transition-all duration-300 hover:-translate-y-1 ${
              plan.featured
                ? "bg-[#141417] border border-blue-400/20"
                : "bg-[#0f0f11] border border-white/10 hover:border-blue-400/10"
            } ${isActive ? "ring-1 ring-blue-400/30" : ""}`}
          >
            {/* Most Popular badge */}
            {plan.featured && !isActive && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-400 text-[#0a0a0b] text-xs font-bold tracking-wide uppercase px-3.5 py-1 rounded-full whitespace-nowrap">
                Most Popular
              </span>
            )}
            ...
            <p className="text-sm text-blue-400 mb-7">{plan.credits}</p>
